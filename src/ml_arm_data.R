...
columns_to_discretize <- c( "highway_fuel_economy", "engine_cylinders", "horsepower", "torque_lbft", "mileage", "year", "wheelbase", "price")
...
breaks <- list(
  ...
  year = quantile(df_tx$year, probs = c(0, 0.1, 0.7, 1)),
  wheelbase = quantile(df_tx$wheelbase, probs = c(0, 0.25, 0.5, 0.75, 1)),
  price = quantile(df_tx$price, probs = c(0, 0.25, 0.5, 0.75, 1))
)

# Column discretization and conversion to string labels
label_mappings <- list(
  highway_fuel_economy = c("Poor", "Average", "Good"),
...
df_transactions <- df_tx %>%
  select(make_name, horsepower, torque_lbft, engine_cylinders, body_type, fuel_type, mileage, year, has_accidents, wheelbase, price)
